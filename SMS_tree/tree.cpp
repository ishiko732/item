...
tree::tree() {
    root = (stuNode *) (malloc(sizeof(stuNode)));
    if (root == nullptr) {
        perror("未能申请到二叉树结点\n");
        return;
    } else {
        root->rchild = nullptr;
        root->lchild = nullptr;
        root->student = nullptr;
    }
}

stuNode *tree::setroot(struct stu *stu) {
    root->student = stu;
    return root;
}
...
stuNode *tree::insert(stuNode *root1, stuNode *stu) {
    ...
    int x = strcmp(root1->student->sno, stu->student->sno);//0 相等 <0右大，>0左大
    if (x < 0) {//插入右边
    ...
    x = strcmp(root1->student->sno, stu->student->sno);
    if (h == 2)//左不平衡
    {
...
stuNode *tree::find(stuNode *root1, char *sno) {//搜索查询学生信息
    ...
    if (x > 0) {
        return find(root->lchild, sno);
    } else if (x < 0) {
        return find(root->rchild, sno);
    }
    return root;
}
...
void tree::print(stuNode *root1) {
    ...
    printf("学号：%s,学生姓名：%s,性别：%s,年龄：%d，地区：%s，专业：%s\n",
           root1->student->sno, root1->student->name, root1->student->sex, root1->student->age, root1->student->region,
           root1->student->pro);
    print(root1->rchild);
}
...
stuNode *tree::remove(stuNode *root1, char *sno) {//删除学生成绩信息
    ...
    if (x == 0) {
        ...
        } else if (root1->lchild != nullptr)//如果有左子树
        {
            ...
            root1->student=swap;
            root1->lchild = remove(root->lchild, sno);//递归的删除掉重复出来的这个节点,将左子树的数据更新进来
        } else//如果只有右子树
        {
            ...
            tmp->student=root1->student;
            root1->student=swap;
            root1->rchild = remove(root->lchild, sno);//递归的删除掉重复出来的这个节点,将右子树的数据更新进来
        }
    } else if (x > 0)//如果删除的数据比这个节点要小，则继续往左边去删除节点
    {
        root1->lchild = remove(root->lchild, sno);
    } else//如果删除的数据比这个节点要大，则继续往右边去删除
    {
        root1->rchild = remove(root->lchild, sno);
    }
