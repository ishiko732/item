...
public class Server {
    ...
    public Server() {
        ...
        while (true) {
            try {
                @SuppressWarnings("ConstantConditions") //idea提示 忽略server.accept()的NullPointerException
...
//                    dos.writeUTF("get-chat[Server],send=[登录失败,原因是用户已登录];" );
    ...
    public static void sendAttack(User user, String message) {
        ...
        if (user.getUID().equals(userUID[0]) && (userMap.containsKey(userUID[1]) || "Server".equals(userUID[1]))) {//发送信息到user2
            try {
                //对于发起者 进行等待
                //对于被邀请 发起询问用户命令
...
//                userMap.get(userUID[0]).getDos().writeUTF(sendMessage);//发送等待命令
                String sendMessage = "get-attack[" + userUID[0] + "],command=[game],send=[请求对战];";
