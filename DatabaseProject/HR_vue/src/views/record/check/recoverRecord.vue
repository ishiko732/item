<template>
  <div>
    ...
    <div v-if="records != null">
      ...
      <el-tag type="danger">当前恢复的人力资源档案总数<span>{{records.length}}</span>例</el-tag>
      <el-table
        :data="records"
        style="width: 100%">
        <el-table-column
    ...
    <el-dialog title="复核信息" :visible.sync="dialogVisible">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="warning" @click="checkRecord_success">恢复</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
<script>
import { check,checkMessageById,checkById,deleteById,getPositionByPid, selectClassification, recoverById } from '@/api/record'
export default {
  name: "recoverRecord",
  data(){
    return {
  ...
  methods:{
    ...
    checkRecord_success(){
      recoverById(this.ruleForm.id).then(response => {
        const { data } = response
